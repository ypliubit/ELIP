...
from .model import CLIP, CustomTextCLIP, CLIPWithClass, convert_weights_to_lp, convert_to_custom_text_state_dict,\
...
def load_checkpoint(model, checkpoint_path, strict=False):
    ...
    for k, v in zip(state_dict.keys(), state_dict.values()):
        if k in model.state_dict():
            if v.size() == model.state_dict()[k].size():
                ...
                logging.info(f'Size mismatching key: {k} size mismatch!')
        else:
            new_state_dict[k] = v
    ...
    for key in incompatible_keys.unexpected_keys:
        logging.info(f"Unexpected key: {key}")
...
def create_model(
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if custom_text:
            ...
            if with_class_label:
                model = CLIPWithClass(**model_cfg, cast_dtype=cast_dtype)
            else:
...
def create_model_and_transforms(
    ...
    model = create_model(
        ...
        with_class_label=with_class_label,
        **model_kwargs,
    )
