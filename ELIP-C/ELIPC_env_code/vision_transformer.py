...
__all__ = ['VisionTransformer', 'TextGuidedPromptedVisionTransformer', 'TextGuidedPromptedVisionTransformerSO']  # model_registry will add each entrypoint fn to this
...
class TextGuidedPromptedVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        in_dim, hidden_dim, bottleneck_dim = 768, 1024, self.width
        num_mapped_vpt = 10 # self.prompt_config["num_mapped_vpt"]
    ...
    def forward_features(self, x: torch.Tensor, text_feat: torch.Tensor) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        ...
        x = torch.cat((
            x[:, :1, :],
            self.prompt_dropout(self.prompt_proj(self.vpt_generator(text_feat).reshape(B, -1, self.width))),
            x[:, 1:, :]
        ), dim=1)
    ...
    def forward(self, x: torch.Tensor, text_feat: torch.Tensor) -> torch.Tensor:
        # ipdb.set_trace()
        x = self.forward_features(x, text_feat)
        x = self.forward_head(x)
        return x
...
class TextGuidedPromptedVisionTransformerSO(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        in_dim, hidden_dim, bottleneck_dim = 1152, 1024, self.width
        num_mapped_vpt = 10 # self.prompt_config["num_mapped_vpt"]
        self.vpt_generator = nn.Sequential(*[
            nn.Linear(in_dim, hidden_dim*num_mapped_vpt),
            nn.GELU(),
...
class TextGuidedPromptedVisionTransformerG(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        in_dim, hidden_dim, bottleneck_dim = 1536, 1024, self.width
        num_mapped_vpt = 10 # self.prompt_config["num_mapped_vpt"]
        self.vpt_generator = nn.Sequential(*[
            nn.Linear(in_dim, hidden_dim*num_mapped_vpt),
            nn.GELU(),
...
class TextGuidedPromptedVisionTransformer_MBCLIP(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        in_dim, hidden_dim, bottleneck_dim = 512, 1024, self.width
        num_mapped_vpt = 10 # 5 # self.prompt_config["num_mapped_vpt"]
    ...
    def pool(self, x: torch.Tensor, pool_type: Optional[str] = None) -> torch.Tensor:
        ...
        # print(self.attn_pool.pool, 'self.attn_pool.pool')
        if self.attn_pool is not None:
            # print(x.shape, 'x.shape')
            x = self.attn_pool(x)
            # print(x.shape, 'x.shape')
            return x
        pool_type = self.global_pool if pool_type is None else pool_type
        # print('self.global_pool', self.global_pool)
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        ...
        x = self.head_drop(x)
        # print(x.shape, 'x.shape')
        # print(pre_logits, 'pre_logits')
        # print(self.head(x).shape, 'self.head(x).shape')
        return x if pre_logits else self.head(x)
...
default_cfgs = {
    ...
    'vit_base_patch16_siglip2_224.webli': _cfg(
        hf_hub_id='timm/ViT-B-16-SigLIP2',
        hf_hub_filename='open_clip_pytorch_model.bin',
        num_classes=0),
...
def _create_vision_transformer_tgvpt(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    out_indices = kwargs.pop('out_indices', 3)
    if 'flexi' in variant:
        # FIXME Google FlexiViT pretrained models have a strong preference for bilinear patch / embed
        # interpolation, other pretrained models resize better w/ anti-aliased bicubic interpolation.
...
def _create_vision_transformer_tgvpt_so(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    out_indices = kwargs.pop('out_indices', 3)
    if 'flexi' in variant:
        # FIXME Google FlexiViT pretrained models have a strong preference for bilinear patch / embed
        # interpolation, other pretrained models resize better w/ anti-aliased bicubic interpolation.
    ...
    return build_model_with_cfg(
        TextGuidedPromptedVisionTransformerSO,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_vision_transformer_tgvpt_g(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    out_indices = kwargs.pop('out_indices', 3)
    if 'flexi' in variant:
        # FIXME Google FlexiViT pretrained models have a strong preference for bilinear patch / embed
        # interpolation, other pretrained models resize better w/ anti-aliased bicubic interpolation.
    ...
    return build_model_with_cfg(
        TextGuidedPromptedVisionTransformerG,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
@register_model
def vit_base_patch16_siglip2_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer_tgvpt(
        'vit_base_patch16_siglip2_224', pretrained=pretrained, **dict(model_args, **kwargs))
    #model = _create_vision_transformer_tgvpt(
    #    'vit_base_patch16_siglip2_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def vit_giantopt_patch16_siglip_384(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    model = _create_vision_transformer_tgvpt_g(
...
def vit_so400m_patch14_siglip_384(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    model = _create_vision_transformer_tgvpt_so(
