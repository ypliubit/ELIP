...
from .vision_transformer import VisionTransformer, TextGuidedPromptedVisionTransformer_MBCLIP
...
def _create_vision_transformer_hybrid_tgvpt(variant, backbone, embed_args=None, pretrained=False, **kwargs):
    out_indices = kwargs.pop('out_indices', 3)
    embed_args = embed_args or {}
    embed_layer = partial(HybridEmbed, backbone=backbone, **embed_args)
    kwargs.setdefault('embed_layer', embed_layer)
    kwargs.setdefault('patch_size', 1)  # default patch size for hybrid models if not set
    return build_model_with_cfg(
        TextGuidedPromptedVisionTransformer_MBCLIP,
        variant,
        pretrained,
        pretrained_filter_fn=checkpoint_filter_fn,
        feature_cfg=dict(out_indices=out_indices, feature_cls='getter'),
...
def vit_base_mci_224(pretrained=False, **kwargs) -> VisionTransformer:
    ...
    #     pretrained=pretrained, **dict(model_args, **kwargs)
    # )
    model = _create_vision_transformer_hybrid_tgvpt(
        'vit_base_mci_224', backbone=backbone, embed_args=dict(proj=False),
        pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
