...
class RetrievalDatasetCSV(BaseDataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
    ...
    def __getitem__(self, index):
        ...
        #image_path = os.path.join(self.vis_root, ann['filepath\ttitle'].split('\t')[0])
        image_path = os.path.join(self.vis_root, ann['filepath'])
        image = Image.open(image_path).convert("RGB")

        image = self.vis_processor(image)
        caption = self.text_processor(ann['title'])
        #caption = self.text_processor(ann['filepath\ttitle'].split('\t')[1])
