...
class ResidualAttentionBlock1(nn.Module):
    ...
    def attention(self, x: torch.Tensor, attn_mask: torch.Tensor):
        self.attn_mask = attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
...
class Transformer1(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, use_grad_checkpointing=False):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock1(width, heads, attn_mask, use_grad_checkpointing and i>12) for i in range(layers)])

    def forward(self, x: torch.Tensor, attn_mask: torch.Tensor):
        # ipdb.set_trace()
        for s in self.resblocks:
            x = s(x, attn_mask)
        return x
