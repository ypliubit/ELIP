...
from torch.utils.data import DataLoader, DistributedSampler, DistributedHarderSampler
...
class RunnerBase:
    ...
    @property
    def val_freq(self):
        val_freq = self.config.run_cfg.get("val_freq", 5)
        ...
        def _create_loader(dataset, num_workers, bsz, is_train, collate_fn):
            ...
            if isinstance(dataset, ChainDataset) or isinstance(
                ...
                if self.use_distributed:
                    ...
                    cur_config = OmegaConf.load(self.config.args.cfg_path)
                    if cur_config['run']['hard_sample']:
                        sampler = DistributedHarderSampler(
                            dataset,
                            shuffle=is_train,
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        state_dict = checkpoint["model"]
        from lavis.models.eva_vit import interpolate_pos_embed2
        interpolate_pos_embed2(self.unwrap_dist_model(self.model).visual_encoder, state_dict)
        self.unwrap_dist_model(self.model).load_state_dict(state_dict)
